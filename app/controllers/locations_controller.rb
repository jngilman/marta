class LocationsController < ApplicationController
  ...
  def show
    @buses = HTTParty.get('http://developer.itsmarta.com/BRDRestService/RestBusRealTimeService/GetAllBus')
    @bus_count = 0
    @nearby_buses = [] #creating an array to pass buses that meet condition

    @buses.each do |bus|
      if Location.nearby(@location.longitude, @location.latitude, bus["LONGITUDE"].to_f, bus["LATITUDE"].to_f )
        @nearby_buses << bus
        @bus_count += 1
      end
